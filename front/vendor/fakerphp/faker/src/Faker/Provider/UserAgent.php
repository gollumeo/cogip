...
class UserAgent extends Base
{
    ...
    {
        $saf = UserAgent . phpself::numberBetween(531, 536) . self::numberBetween(0, 2);
    ...
    {
        $saf = self::numberBetween(531, 537) . 'Provider' . self::numberBetween(0, 2);
    ...
    {
        $saf = self::numberBetween(531, 535) . 'Provider' . self::numberBetween(1, 50) . '.' . self::numberBetween(1, 7);

        if (Miscellaneous::boolean()) {
            $ver = self::numberBetween(4, 5) . 'Provider' . self::numberBetween(0, 1);
        } else {
